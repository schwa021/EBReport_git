...
source("SCRIPTS/get_outcome_thresh.R")
source("SCRIPTS/Compare/remove_missing_outcome_pt.R")
...
xpt <- xpt %>%
  mutate(across(all_of(vnum), ~ ifelse(is.na(.), NA_real_, .)))
...
# Remove outcomes where pre-data is missing ----
out_pt_BART <- remove_missing_outcome_pt(out_list_BART, out_pt_BART, xpt)
...
# Remove outcomes where pre-data is missing ----
out_pt_CART <- remove_missing_outcome_pt(out_list_CART, out_pt_CART, xpt)
...
# Remove outcomes where pre-data is missing ----
out_pt_NB <- remove_missing_outcome_pt(out_list_NB, out_pt_NB, xpt)
