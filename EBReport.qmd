...
if(file.exists("DATA/preprocessed_data_current.RDS")){
  ...
  dat0 <- readRDS("DATA/FDpp_2026-04-06.RDS")
...
datpre <- readRDS("DATA/FD_2026-04-06.RDS")
...
for (s in surglist) {
  if(file.exists(glue("OUTPUT/Propensity_Models/prop_{s}.RDATA"))){
    # Comment above and Uncomment below to re-run all propensity models
    # if(FALSE){
...
# Get dimensionless energy data
fd$FD <- get_O2_Z(fd$FD)
if(!is.null(fd$FDpp)){fd$FDpp <- get_O2_Z(fd$FDpp)}
...
deletion_result <- file.remove(mp4_files)
```
...
v <- c("O2_NDspeed", "NETND_OXYCONS_Z")
cap = ""
p <- plot_development(fd, dat, v, v, cap, devdat)
p
```
...
```{r}
#| include: false
source("SCRIPTS/get_pp.R")

rootpath <- "L:/Motion Analysis Lab/Electronic Report"
printable <- list.files(
  path = glue("{rootpath}/{params$MRN}"), 
...
output_path <- "OUTPUT/pp_plot.png"

# Only proceed if PDF files were actually found
if (length(printable) > 0) {
  # Date parsing
  date_strings <- regmatches(printable, regexpr("[A-Za-z]{3}[0-9]{4}", printable))
  
  # Ensure date_strings matches printable length before ordering
  if (length(date_strings) == length(printable)) {
    date_strings <- as.Date(paste0("01", date_strings), format = "%d%b%Y")
    printable <- printable[order(date_strings)]
  }

  pp_imgs <- map(printable, get_pp)
  pp_imgs <- pp_imgs[!sapply(pp_imgs, is.null)]

  # CHECK: Only attempt to stitch images if the list is not empty
  if (length(pp_imgs) > 0) {
    cols <- 2
    rows <- split(pp_imgs, ceiling(seq_along(pp_imgs) / cols))

    row_strips <- lapply(rows, function(r) {
      # Ensure we have image objects before calling append
      magick::image_append(do.call(c, r), stack = FALSE)
    })

    final_grid <- magick::image_append(do.call(c, row_strips), stack = TRUE)
    
    # Save the successful plot
    magick::image_write(final_grid, output_path)
  } else {
    # If no images found in PDFs, create a blank placeholder or delete old file
...
#| out-width: "6.5in"
#| crop: TRUE
#| eval: !expr file.exists("OUTPUT/pp_plot.png")
knitr::include_graphics("OUTPUT/pp_plot.png")
```
:::
:::
...
<!-- Remove newly created videos -->
```{r}
#| label: clean-up-videos
#| include: false
# mp4_files <- list.files(
#   path = ".",          
#   pattern = "\\.mp4$", 
