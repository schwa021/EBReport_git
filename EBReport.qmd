...
<!-- The similarity of the patient to the historical data used to fit the propensity and outcome models.  -->

<!-- > <center>![](DATA/caution.jpeg){width="2in"}\ -->
<!-- > Models with **poor applicability** should be interpreted with extra caution.</center> -->

```{r}
#| label: compute-model-applicability
#| cache: false
#| include: false
source("SCRIPTS/outlier_isoforest.R")
source("SCRIPTS/format_applicability.R")
# Compute anomaly score using isolation forest for dx and all surgeries -----
modlist <-  c("dx", surglist)
res <- modlist %>% map(\(x) outlier_isoforest(xpt, dat, x), .progress = TRUE)
applicability <- format_applicability(res, surglist, xpt)
applicability
```
...
for (kk in 1:length(shap_tables)) {
  ...
  # Save for external use -----
  if(params$figsave){
    gtsave(shap_tables[[kk]], glue("FIGURES/Clin_Reasoning_{surglist[kk]}.png"), expand = c(10, 40, 10, 00))
  }
}
