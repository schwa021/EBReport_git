...
```{r}
#| label: build-outcome-profile
#| cache: false

source("SCRIPTS/build_outcome_profile.R")
outcome_profile <- build_outcome_profile(out_all)

```

### Outcome Profile

**EXPERIMENTAL - Please Consult Outcome Details Before Operating**

The outcome profile shows the probability that a treatment effect ($\Delta_{treated}-\Delta_{control}$) will surpass a chosen threshold. The *variables* and *thresholds* are pre-determined by a panel of clinical experts.

Example: Femoral Derotation Osteotomy  
-   **Structure**: Trochanteric Prominence Angle, $\Delta < -10^\circ$\
-   **Kinematics**: Mean Stance Hip Angle Transverse Plane $\Delta < -5^\circ$\

::: {.panel-tabset .nav-pills}
```{r}
#| label: display-outcome-profile
#| results: asis

for (kk in 1:length(outcome_profile)) {
  sdstr <- ifelse(kk==1, "Left", "Right")
  cat("####", sdstr, "\n\n")
  print(outcome_profile[[kk]])
  cat("\n\n")
}
