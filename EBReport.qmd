...
# Save xpt with params$MRN and params$Event_Date in name -----
saveRDS(xpt, glue("EBData_{params$MRN}_{params$Event_Date}.RDS"))
...
# Deidentified videos created separetly and stored in VIDEO/MRN/
...
# This is the code to build the outcome prediction models -----
...
# # Compute Models -----
# usegait = TRUE
# bartlist <- v %>% map(\(v) build_pred_outcome_BART(dat, v, usegait), .progress = TRUE)
# names(bartlist) <- v
...
usegait <- params$usegait
if(usegait){
  bartlist_pred <- readRDS("DATA/outcome_models_bartlist.RDS")
} else {
  bartlist_pred <- readRDS("DATA/outcome_models_bartlist_nogait.RDS")
}
