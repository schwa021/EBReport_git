...
if(file.exists("DATA/preprocessed_data_2024-04-19.RDS")){
  ...
  temp <- get_meanscores(dat, "Pre")
  dat$avgspa <- temp$meanspa
  dat$avgwb <- temp$meanwb
  dat$avgnwb <- temp$meannwb
  
  temp <- get_meanscores(dat, "Post")
  dat$avgspaPost <- temp$meanspa
  dat$avgwbPost <- temp$meanwb
  dat$avgnwbPost <- temp$meannwb
} else {
  ...
  temp <- get_meanscores(dat, "Post")
  dat$avgspaPost <- temp$meanspaPost
  dat$avgwbPost <- temp$meanwbPost
  dat$avgnwbPost <- temp$meannwbPost
}
...
# List of surgeries -----
surglist <- c("Femoral_Derotation_Osteotomy", "Tibial_Derotation_Osteotomy", "Psoas_Release", "Hams_Lengthening", "Adductor_Release", "Gastroc_Soleus_Lengthening", "DFEO", "Patellar_Advance",  "Foot_and_Ankle_Bone", "Foot_and_Ankle_Soft_Tissue","Neural_Rhizotomy", "Rectus_Transfer")
...
xpt <- 
  ...
  temp <- get_meanscores(xpt, "Pre")
  xpt$avgspa <- temp$meanspa
  xpt$avgwb <- temp$meanwb
  xpt$avgnwb <- temp$meannwb
...
if(!is.null(datc3d_afo)){
  ...
  if(nrow(temp) > 0){
    ...
    pLafo <- plot_patient_knmafo(datc3d, datc3d_afo, xpt, "L")
    pRafo <- plot_patient_knmafo(datc3d, datc3d_afo, xpt, "R")
  } else {
    pLafo <- NULL
    pRafo <- NULL
  }
